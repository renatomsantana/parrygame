/*
 * roster.c - A Trilha dos Doze Aprendizes: onze aprendizes de Hattori Hanzo e
 * Oboro, o mestre das doze posturas. Preparações e janelas em segundos.
 */
#include "core.h"
...
static const MasterProfile ROSTER[ROSTER_SIZE] = {
    {
        .id = 1, .name = "raijin", .style = "postura do touro", .title = "O Primeiro da Fila", .venue = "Pátio do dojo",
        .special = "Odachi pesada: golpes lentos, diretos, que avisam antes de chegar.",
        .arena = ARENA_DOJO, .posture = 300, .hitsToFall = 50, .cueVisual = 1, .cueAudio = 1, .tint = 0xE8D8C8FF,
        .stances = {STANCE("", 0.090f, 0.220f, 1.20f, 1.05f, 1.15f)}, .stanceCount = 1,
        .moves = {
            {"corte do touro", 1, {0}, 3.0f, -1, 0, LOOK_HIGH},
            {"investida dupla", 2, {0.85f}, 1.5f, -1, 0, LOOK_LOW},
        },
        .moveCount = 2,
        .intro = {{"raijin", "oboro ganhou cada coisa que tem, com suor. E você, ganhou o quê? Um velho que te achou na estrada?"},
                  {"musashi", "Uma pergunta. Vim ver se alguém aqui sabe a resposta."}}, .introCount = 2,
        .outro = {{"raijin", "Não foi você que foi rápido. Fui eu que corri."}}, .outroCount = 1,
    ...
    {
        .id = 2, .name = "shizuku", .style = "postura da água", .title = "A que Observa", .venue = "Cachoeira do Trovão",
        .special = "Florete: estocadas retas, uma ou duas, sem curva.",
        .arena = ARENA_CACHOEIRA, .posture = 300, .hitsToFall = 40, .cueVisual = 1, .cueAudio = 1, .tint = 0xC8E8FFFF,
        .stances = {STANCE("", 0.085f, 0.210f, 0.95f, 0.85f, 1.00f)}, .stanceCount = 1,
        .moves = {
        ...
        .intro = {{"shizuku", "A água toma a forma de qualquer copo. Passei anos tentando tomar a forma do mestre. Nunca coube."},
                  {"musashi", "Talvez ele não fosse um copo."}}, .introCount = 2,
    ...
    {
        .id = 3, .name = "kage", .style = "postura da noite", .title = "O que Desconfia", .venue = "Bambuzal à meia-noite",
        .special = "Duas adagas: às vezes as lanternas se apagam e só o som avisa.",
        .arena = ARENA_BAMBUZAL, .posture = 330, .hitsToFall = 35, .cueVisual = 1, .cueAudio = 1, .tint = 0xB8B0E0FF,
        .blackoutChance = 0.3f,
        .stances = {STANCE("", 0.080f, 0.200f, 0.90f, 0.80f, 1.00f)}, .stanceCount = 1,
        .moves = {
            {"sombra", 1, {0}, 2.0f, -1, 0, LOOK_LOW},
            {"presas", 2, {0.45f}, 2.0f, -1, 0, LOOK_HIGH},
        },
        .moveCount = 2,
        .intro = {{"kage", "Todo mundo viu oboro vencer. Ninguém viu o que hanzo fez um instante antes."},
                  {"musashi", "E o que ele fez?"}}, .introCount = 2,
        .outro = {{"kage", "Abriu a guarda. De propósito. Guarde isso, garoto. E não conte a oboro que eu vi."}}, .outroCount = 1,
    ...
    {
        .id = 4, .name = "daichi", .style = "postura da terra", .title = "O que Segue a Tradição", .venue = "Celeiro ao entardecer",
        .special = "Espada pesada: o segundo golpe sempre demora.",
        .arena = ARENA_CELEIRO, .posture = 330, .hitsToFall = 30, .cueVisual = 1, .cueAudio = 1, .tint = 0xFFD8A8FF,
        .stances = {STANCE("", 0.075f, 0.190f, 1.20f, 1.00f, 1.30f)}, .stanceCount = 1,
        .moves = {
            {"rocha", 1, {0}, 2.0f, -1, 0, LOOK_HIGH},
            {"desabamento", 2, {1.00f}, 2.0f, -1, 0, LOOK_LOW},
            {"terremoto", 3, {0.80f, 0.50f}, 1.0f, -1, 0, LOOK_HIGH},
        },
        .moveCount = 3,
        .intro = {{"daichi", "A tradição diz que eu sigo quem venceu o mestre. Ela não diz que eu preciso gostar."},
                  {"musashi", "Nem que precisa perder de propósito."}}, .introCount = 2,
    ...
    {
        .id = 5, .name = "hayate", .style = "postura do vento", .title = "O Impaciente", .venue = "Teto do trem noturno",
        .special = "Katana leve: o ritmo muda de direção sem avisar.",
        .arena = ARENA_TREM, .posture = 360, .hitsToFall = 25, .healsOnHit = true, .cueVisual = 1, .cueAudio = 1, .tint = 0xD0FFE8FF,
        .rhythmJitter = 0.12f,
        .stances = {STANCE("", 0.072f, 0.185f, 0.85f, 0.75f, 1.00f)}, .stanceCount = 1,
        .moves = {
        ...
        .intro = {{"hayate", "hanzo passou a vida dizendo que oboro não estava pronto. oboro provou que estava. Agora prova você."},
                  {"musashi", "Não vim provar nada a você."}}, .introCount = 2,
    ...
    {
        .id = 6, .name = "genbu", .style = "postura da tartaruga", .title = "O que Tem Pena", .venue = "Jardim de pedra e vidro",
        .special = "Escudo e espada curta: demora a sair do casco, sai rápido.",
        .arena = ARENA_JARDIM, .posture = 360, .hitsToFall = 22, .healsOnHit = true, .cueVisual = 1, .cueAudio = 1, .tint = 0xD8E0C8FF,
        .stances = {STANCE("", 0.070f, 0.180f, 1.30f, 1.20f, 1.40f)}, .stanceCount = 1,
        .moves = {
            {"casco", 1, {0}, 2.0f, -1, 0, LOOK_THRUST},
            {"mordida", 2, {0.42f}, 2.0f, -1, 0, LOOK_THRUST},
            {"carapaça", 3, {0.90f, 0.42f}, 1.0f, -1, 0, LOOK_LOW},
        },
        .moveCount = 3,
        .intro = {{"genbu", "Sabe por que a tartaruga vive tanto? Ela não tem pressa de chegar a lugar nenhum. oboro tinha."},
                  {"musashi", "Eu não tenho."}}, .introCount = 2,
    ...
    {
        .id = 7, .name = "enjin", .style = "postura da chama", .title = "O que Odeia", .venue = "Forja dentro da cratera",
        ...
        .stances = {STANCE("", 0.065f, 0.170f, 0.70f, 0.80f, 0.65f)}, .stanceCount = 1,
        .moves = {
        ...
        .intro = {{"enjin", "Eu odeio o homem que oboro virou. E odeio o velho por não ter impedido. Se você não aguentar o meu fogo, não aguenta o dele."},
                  {"musashi", "Então queima."}}, .introCount = 2,
    ...
    {
        .id = 8, .name = "suiren", .style = "postura do mar", .title = "O Amigo de Oboro", .venue = "Porto dos tambores",
        .special = "Lança: as ondas vêm cada vez mais rápido e depois recuam.",
        .arena = ARENA_PORTO, .posture = 390, .hitsToFall = 18, .healsOnHit = true, .cueVisual = 1, .cueAudio = 1, .tint = 0xC8D8FFFF,
        .accelSteps = 5, .accelFactor = 0.82f,
        .stances = {STANCE("", 0.062f, 0.165f, 1.20f, 1.05f, 1.15f)}, .stanceCount = 1,
        .moves = {
        ...
        .intro = {{"suiren", "Você tem os olhos que ele tinha quando chegou ao dojo. Antes de querer ser alguém."},
                  {"musashi", "O que aconteceu com ele?"}}, .introCount = 2,
    ...
    {
        .id = 9, .name = "karasu", .style = "postura do corvo", .title = "O que Aposta", .venue = "Cobertura na chuva",
        .special = "Espada e adaga: sempre uma pausa antes do último golpe.",
        .arena = ARENA_COBERTURA, .posture = 420, .hitsToFall = 15, .healsOnHit = true, .cueVisual = 1, .cueAudio = 1, .tint = 0xC8C8D8FF,
        .stances = {STANCE("", 0.060f, 0.160f, 0.83f, 1.13f, 0.87f)}, .stanceCount = 1,
        .moves = {
        ...
        .intro = {{"karasu", "Eu sigo quem ganha. Hoje é oboro. Não se ofenda se eu apostar contra você."},
                  {"musashi", "Aposta. Eu não jogo."}}, .introCount = 2,
    ...
    },
    {
        ...
        .arena = ARENA_SALAO, .posture = 420, .hitsToFall = 12, .healsOnHit = true, .cueVisual = 1, .cueAudio = 1, .tint = 0xE0D0F0FF,
        .stances = {STANCE("", 0.058f, 0.155f, 0.80f, 0.90f, 0.75f)}, .stanceCount = 1,
        .moves = {
        ...
        .intro = {{"arashi", "O velho envelheceu e foi superado. É assim que o mundo anda. Você é só o eco de quem já perdeu."},
                  {"musashi", "Um eco ainda chega longe."}}, .introCount = 2,
    ...
    {
        .id = 11, .name = "jinshi", .style = "postura da montanha", .title = "O que Não Perdoou", .venue = "Encosta da serra",
        .special = "Cajado de ferro: na montanha o som não chega, só o corpo avisa.",
        ...
        .intro = {{"jinshi", "hanzo falhou com oboro. oboro falhou com todos nós. Eu fiquei na montanha esperando alguém que não falhasse."},
                  {"musashi", "Eu ainda não terminei."}}, .introCount = 2,
    ...
    {
        .id = 12, .name = "oboro", .style = "o mestre das doze posturas", .title = "O Mestre das Doze Posturas", .venue = "O dojo de Hanzo, no alto da serra",
        .special = "Domina as onze posturas e troca de uma para outra; três selos.",
        ...
        .stances = {
            STANCE("postura do touro", 0.070f, 0.180f, 1.05f, 0.95f, 1.05f),
            STANCE("postura da água", 0.065f, 0.175f, 0.85f, 0.80f, 0.90f),
            STANCE("postura da noite", 0.062f, 0.170f, 0.80f, 0.75f, 0.90f),
            STANCE("postura da terra", 0.060f, 0.165f, 1.05f, 0.90f, 1.15f),
            STANCE("postura do vento", 0.058f, 0.160f, 0.75f, 0.70f, 0.90f),
            STANCE("postura da tartaruga", 0.056f, 0.155f, 1.15f, 1.05f, 1.25f),
            STANCE("postura da chama", 0.054f, 0.150f, 0.65f, 0.72f, 0.60f),
            STANCE("postura do mar", 0.052f, 0.145f, 1.05f, 0.95f, 1.00f),
            STANCE("postura do corvo", 0.050f, 0.140f, 0.75f, 1.00f, 0.80f),
            STANCE("postura da tempestade", 0.048f, 0.135f, 0.72f, 0.80f, 0.68f),
        ...
        .moves = {
            {"eco do touro", 2, {0.85f}, 1.0f, 0, 0, LOOK_LOW},
            {"eco da água", 2, {0.50f}, 1.0f, 1, 0, LOOK_THRUST},
            {"eco da noite", 2, {0.45f}, 1.0f, 2, 0, LOOK_HIGH},
            {"eco da terra", 3, {0.80f, 0.50f}, 1.0f, 3, 0, LOOK_HIGH},
            {"eco do vento", 3, {0.50f, 0.90f}, 1.0f, 4, 0, LOOK_LOW},
            {"eco da tartaruga", 3, {0.90f, 0.42f}, 1.0f, 5, 0, LOOK_LOW},
            {"eco da chama", 3, {0.45f, 0.45f}, 1.0f, 6, 0, LOOK_HIGH},
            {"eco do mar", 4, {0.55f, 0.50f, 0.45f}, 1.0f, 7, 0, LOOK_HIGH},
            {"eco do corvo", 3, {0.50f, 0.90f}, 1.0f, 8, 0, LOOK_LOW},
            {"eco da tempestade", 3, {0.40f, 0.40f}, 1.0f, 9, 0, LOOK_LOW},
            {"eco da montanha", 4, {0.50f, 0.50f, 0.90f}, 1.0f, 10, 0, LOOK_HIGH},
            {"doze posturas", 5, {0.40f, 0.40f, 0.40f, 0.90f}, 1.0f, -1, 2, LOOK_HIGH},
        },
        .moveCount = 12,
        .intro = {{"oboro", "Onze posturas. Aprendi todas. Dominei todas. E ele escolheu você, que não sabe o nome de nenhuma."},
                  {"musashi", "Eu não aprendi posturas. Aprendi a esperar."},
                  {"oboro", "Então espera. Vou te mostrar as onze, uma de cada vez."}}, .introCount = 3,
        .outro = {{"oboro", "Ele me deu aquela abertura... e eu passei a vida achando que tinha vencido. Nunca entendi nada."}}, .outroCount = 1,
        .sensei = {{"hanzo", "Ele aprendeu as onze posturas, mas usa todas do mesmo jeito: com pressa."},
...
static const char *LORE[LORE_PAGES] = {
    ...
    "Já velho, fundou um dojo no alto da serra e reuniu doze aprendizes. Procurava um sucessor.",
    ...
    "Anos depois, na estrada, Hanzo encontrou um menino chamado Musashi. Pela primeira vez, acreditou estar "
    "diante de alguém capaz de compreender a Arte do Aparar.",
    "Agora Musashi sobe a Trilha dos Doze Aprendizes. Não para vingar ninguém. Para mostrar que o estilo de "
    "Hanzo é inabalável.",
